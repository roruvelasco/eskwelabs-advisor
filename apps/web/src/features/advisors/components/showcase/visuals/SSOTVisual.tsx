...
export function SSOTVisual({
  ...
  const chips = ['Definitions', 'Assumptions', 'Decisions', 'Open Questions', 'Next Steps'];
  const snippets = [
    ...
    <div className="relative" style={{ paddingBottom: '1.5rem' }}>
      ...
      {[2, 1].map((depth) => (
        <motion.div
          ...
          animate={triggered ? { opacity: 1 - depth * 0.3, y: 0 } : { opacity: 0, y: depth * 10 }}
      ...
      <VisualFrame theme={theme} className="relative z-10">
        <div className="mb-4 pb-3" style={{ borderBottom: `1px solid ${theme.dividerColor}` }}>
          <div
          ...
          <div className="font-serif text-base font-medium" style={{ color: theme.headingColor }}>
        ...
        <div className="flex flex-wrap gap-1.5">
          {chips.map((c, i) => (
            <motion.span
              ...
              animate={triggered ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.85 }}
